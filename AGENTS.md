...
internal/
  ...
  world/           # Domain: entities, components, lifecycle  (TBD)
  agent/           # Domain: state machine execution  (TBD)
  storage/         # Infrastructure: SQLite-backed implementations
  ports.go         # Domain ports (interfaces)  (TBD)
```

**Rules:**
- Domain packages (`schema/`, `world/`, `agent/`) import nothing from `storage/`
- `storage/` depends on domain interfaces, never leaks `*sql.DB` upward
- `main.go` and `cmd/` are the composition root — the only place that wires concrete implementations to ports
- Future: `storage/` and filesystem adapters will move under `adapters/` when the hexagonal boundary is fully realised
- No `init()` functions in domain or adapter packages; explicit construction via constructors (Cobra `init()` in `cmd/` is the sole exception for flag registration)
...
cmd/<binary>/
  ...
  wire.go       # Wire provider sets (imports domain + storage)
...
- Integration tests in `storage/` use a fresh in-memory SQLite per test
